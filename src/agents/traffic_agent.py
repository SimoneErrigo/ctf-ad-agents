...
SYSTEM_PROMPT = (
    ...
    "validate_filter. Create the EXACT expression you just verified — do NOT swap in a "
    "different or fancier regex variant at create time (regex backslashes are easily "
    "over-escaped and then silently match NOTHING). Prefer a literal `body icontains "
    "\"<a substring of the REAL observed payload>\"` over `matches` when a substring "
    "already isolates the attack and spares checker traffic. After create_rule, re-run "
    "the confirm list_packets with the rule's STORED expression: it MUST still hit the "
    "cited attack packet — if it now matches nothing, the escaping is wrong, delete and "
    "recreate with the working (verified) expression.\n"
